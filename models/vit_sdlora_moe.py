...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10, 
                 scaling_factor=None, scaling_factor_prev=None):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, task_id, register_hook=False):
        B, N, C = x.shape
        qkv = self.qkv(x)

        new_k, new_v = 0, 0
        for i in range(task_id):
            new_k += self.scaling_factor_prev[i](self.lora_B_k[i](self.lora_A_k[i](x)) / (torch.norm(self.lora_A_k[i].weight) * torch.norm(self.lora_B_k[i].weight)))
            new_v += self.scaling_factor_prev[i](self.lora_B_v[i](self.lora_A_v[i](x)) / (torch.norm(self.lora_A_v[i].weight) * torch.norm(self.lora_B_v[i].weight)))

        new_k += self.scaling_factor[0](self.lora_B_k[task_id](self.lora_A_k[task_id](x)))
        new_v += self.scaling_factor[0](self.lora_B_v[task_id](self.lora_A_v[task_id](x)))
...
class Attention_LoRA_Moe(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10, 
                 scaling_factor=None, scaling_factor_prev=None, use_fofs: bool = True, num_experts: int = 10, top_k: int = 8):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        self.top_k = top_k
        self.jitter_noise = 0.5
        
        # K 和 V 各自的 gate
        self.gate_k = nn.Linear(dim, self.num_experts, bias=False)
        self.gate_v = nn.Linear(dim, self.num_experts, bias=False)
        
        # 为每个专家创建完整的 LoRA (A + B)
        self.lora_A_k = nn.ModuleList([nn.Linear(dim, r, bias=False) for _ in range(self.num_experts)])
        self.lora_B_k = nn.ModuleList([nn.Linear(r, dim, bias=False) for _ in range(self.num_experts)])
        self.lora_A_v = nn.ModuleList([nn.Linear(dim, r, bias=False) for _ in range(self.num_experts)])
        self.lora_B_v = nn.ModuleList([nn.Linear(r, dim, bias=False) for _ in range(self.num_experts)])
        
        # 历史任务的 LoRA（用于增量学习）
        self.lora_A_k_prev = nn.ModuleList()
        self.lora_B_k_prev = nn.ModuleList()
        self.lora_A_v_prev = nn.ModuleList()
        self.lora_B_v_prev = nn.ModuleList()
    ...
    def add_new_task(self, task_id):
        ...
        # 将当前任务的参数移到历史任务中
        if task_id > 0:
            self.lora_A_k_prev.append(self.lora_A_k[task_id - 1])
            self.lora_B_k_prev.append(self.lora_B_k[task_id - 1])
            self.lora_A_v_prev.append(self.lora_A_v[task_id - 1])
            self.lora_B_v_prev.append(self.lora_B_v[task_id - 1])

    def _get_expert_output(self, hidden_states, A_list, B_list, gate, return_all_outputs=False):
        """
        获取专家输出
        ...
        Args:
            ...
            B_list: lora_B 列表
            gate: gate 网络
            return_all_outputs: 是否返回所有专家输出（用于计算 ETF loss）
        
        Returns:
            final_output: [B*N, C]
            routing_weights: [B*N, top_k]
            selected_experts: [B*N, top_k]
            all_expert_outputs: [B*N, num_experts, C] or None
            load_balance_loss: scalar
        """
        batch_size = hidden_states.shape[0]
        hidden_dim = hidden_states.shape[-1]
        
        # 添加 jitter noise（仅训练时）
        ...
        # 路由
        router_logits = gate(hidden_states)  # [B*N, num_experts]
        ...
        if self.training and return_all_outputs:
            # 训练模式：计算所有专家输出
            all_expert_outputs = []
            for i in range(self.num_experts):
                expert_out = B_list[i](A_list[i](hidden_states))
                all_expert_outputs.append(expert_out)
            all_expert_outputs = torch.stack(all_expert_outputs, dim=1)  # [B*N, num_experts, dim]
            
            # 根据权重组合
            token_indices = torch.arange(batch_size, device=hidden_states.device).unsqueeze(1)
            selected_outputs = all_expert_outputs[token_indices, selected_experts]  # [B*N, top_k, dim]
            weighted_outputs = selected_outputs * routing_weights.unsqueeze(-1)
            final_output = weighted_outputs.sum(dim=1)  # [B*N, dim]
            
            return final_output, routing_weights, selected_experts, all_expert_outputs, load_balance_loss
        else:
            # 推理模式：只计算选中的专家
            final_output = torch.zeros(batch_size, hidden_dim, device=hidden_states.device, dtype=hidden_states.dtype)
            
            # 按专家分组处理
            for k in range(self.top_k):
                expert_idx = selected_experts[:, k]  # [B*N]
                weight = routing_weights[:, k:k+1]  # [B*N, 1]
                ...
                for exp_id in unique_experts:
                    ...
                    expert_out = B_list[exp_id](A_list[exp_id](selected_hidden))
                    final_output[mask] += weight[mask] * expert_out
            
            return final_output, routing_weights, selected_experts, None, load_balance_loss

    def forward(self, x, task_id=0, register_hook=False):
        """
        Args:
            x: [B, N, C] 输入特征
            task_id: 当前任务ID
            register_hook: 是否注册hook
        
        Returns:
            x: [B, N, C] 输出特征
            load_balance_loss: 负载均衡损失
            all_expert_outputs_k: K的所有专家输出（用于ETF loss）
            selected_experts_k: 选择的专家索引
        """
        B, N, C = x.shape
        qkv = self.qkv(x)
        
        # 分离 Q, K, V
        k_original = qkv[:, :, self.dim: 2*self.dim]  # [B, N, C]
        v_original = qkv[:, :, 2*self.dim:]           # [B, N, C]
        
        # 展平
        hidden_states_flat_k = k_original.reshape(-1, C)  # [B*N, C]
        hidden_states_flat_v = v_original.reshape(-1, C)  # [B*N, C]
        
        ####################### K 的 MoE 处理 ##############################
        moe_output_k, routing_weights_k, selected_experts_k, all_expert_outputs_k, lb_loss_k = \
            self._get_expert_output(hidden_states_flat_k, self.lora_A_k, self.lora_B_k, self.gate_k, return_all_outputs=True)
        
        ####################### V 的 MoE 处理 ##############################
        moe_output_v, routing_weights_v, selected_experts_v, all_expert_outputs_v, lb_loss_v = \
            self._get_expert_output(hidden_states_flat_v, self.lora_A_v, self.lora_B_v, self.gate_v, return_all_outputs=True)
        
        # 组合负载均衡损失
        total_lb_loss = lb_loss_k + lb_loss_v
        
        # 重塑回原始形状
        moe_output_k = moe_output_k.reshape(B, N, C)
        moe_output_v = moe_output_v.reshape(B, N, C)
        ...
        # 添加历史任务的贡献（如果有）
        if task_id > 0 and len(self.lora_A_k_prev) > 0:
            for i in range(min(task_id, len(self.lora_A_k_prev))):
                # 历史任务的 LoRA（带归一化）
                hist_k = self.lora_B_k_prev[i](self.lora_A_k_prev[i](x))
                hist_k_norm = hist_k / (torch.norm(self.lora_A_k_prev[i].weight) * torch.norm(self.lora_B_k_prev[i].weight) + 1e-6)
                if self.scaling_factor_prev is not None and i < len(self.scaling_factor_prev):
                    new_k += self.scaling_factor_prev[i](hist_k_norm)
                
                hist_v = self.lora_B_v_prev[i](self.lora_A_v_prev[i](x))
                hist_v_norm = hist_v / (torch.norm(self.lora_A_v_prev[i].weight) * torch.norm(self.lora_B_v_prev[i].weight) + 1e-6)
                if self.scaling_factor_prev is not None and i < len(self.scaling_factor_prev):
                    new_v += self.scaling_factor_prev[i](hist_v_norm)
        
        # 添加当前任务的贡献
        if self.scaling_factor is not None and len(self.scaling_factor) > 0:
            curr_k = self.lora_B_k[task_id if task_id < len(self.lora_B_k) else -1](self.lora_A_k[task_id if task_id < len(self.lora_A_k) else -1](x))
            curr_v = self.lora_B_v[task_id if task_id < len(self.lora_B_v) else -1](self.lora_A_v[task_id if task_id < len(self.lora_A_v) else -1](x))
            new_k += self.scaling_factor[0](curr_k)
            new_v += self.scaling_factor[0](curr_v)
        ...
        # 返回与 BaseIndependentMoE 兼容的格式
        # 为了 ETF loss，我们需要返回所有专家的输出（K的即可，或者合并K和V的）
        # 这里返回 K 的所有专家输出用于计算 ETF loss
        return x, total_lb_loss, all_expert_outputs_k, selected_experts_k, all_expert_outputs_v, selected_experts_v
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, n_tasks=10, r=64, scaling_factor=None, scaling_factor_prev=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_LoRA(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, 
                                   n_tasks=n_tasks, r=r, scaling_factor=scaling_factor, scaling_factor_prev=scaling_factor_prev)
    ...
    def forward(self, x, task_id, register_hook=False):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), task_id, register_hook=register_hook)))
...
class Block_MOE(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_LoRA_Moe(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, 
                                   n_tasks=n_tasks, r=r, scaling_factor=scaling_factor, scaling_factor_prev=scaling_factor_prev)
        self.ls1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x, task_id, register_hook=False):
        att, total_lb_loss, all_expert_outputs_k, selected_experts_k, all_expert_outputs_v, selected_experts_v = self.attn(self.norm1(x), task_id, register_hook=register_hook)
...
class ParallelBlock(nn.Module):
    ...
    def forward(self, x):
        # print("self.num_parallel", self.num_parallel)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, block_moe=Block_MOE, n_tasks=10, rank=64, moe_list=None):
        ...
        # sd-lora scaling factors
        self.scaling_factor = nn.ModuleList([ParameterWrapper(nn.Parameter(torch.Tensor([0.8])))])
        self.scaling_factor_prev = nn.ModuleList([ParameterWrapper(nn.Parameter(torch.Tensor([0.8]))) for _ in range(20)])
        ...
        #         init_values=init_values, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, n_tasks=n_tasks, r=rank, 
        #         scaling_factor=self.scaling_factor, scaling_factor_prev=self.scaling_factor_prev)
        #     for i in range(depth)
        # ])

        self.moe_list = moe_list 
        # print("self.moe_list", self.moe_list)
        self.blocks = nn.Sequential()
        for i in range(depth):
            # 这里的block是从
            if i in self.moe_list:
                self.blocks.append(block_moe(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate,
                    init_values=init_values, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, n_tasks=n_tasks, r=rank, 
                    scaling_factor=self.scaling_factor, scaling_factor_prev=self.scaling_factor_prev))
            else:
