...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10):
        ...
        # ========== SD-LoRA style scaling factors (内部创建，不依赖外部) ==========
        # 当前任务的缩放因子
        self.scaling_factor = nn.ModuleList([ParameterWrapper(nn.Parameter(torch.Tensor([0.8])))])
        # 历史任务的缩放因子（最多20个任务）
        self.scaling_factor_prev = nn.ModuleList([ParameterWrapper(nn.Parameter(torch.Tensor([0.8]))) for _ in range(10)])
        # ====================================================================
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        ...
        # ========== SD-LoRA style with scaling factors ==========
        new_k, new_v = 0, 0
        
        # 历史任务：使用 scaling_factor_prev 并除以权重范数归一化
        for i in range(task):
            new_k += (self.scaling_factor_prev[i])(
                self.lora_B_k[i](self.lora_A_k[i](x)) / 
                (torch.norm(self.lora_A_k[i].weight) * torch.norm(self.lora_B_k[i].weight) + 1e-8)
            )
            new_v += (self.scaling_factor_prev[i])(
                self.lora_B_v[i](self.lora_A_v[i](x)) / 
                (torch.norm(self.lora_A_v[i].weight) * torch.norm(self.lora_B_v[i].weight) + 1e-8)
            )
        
        # 当前任务：使用 scaling_factor[0]
        new_k += (self.scaling_factor[0])(self.lora_B_k[task](self.lora_A_k[task](x)))
        new_v += (self.scaling_factor[0])(self.lora_B_v[task](self.lora_A_v[task](x)))
        
        # 将 LoRA 增量加到 k, v 上
        k = k + new_k.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = v + new_v.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # ========================================================
