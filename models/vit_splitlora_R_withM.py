...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10):
        ...
        # ========== LoRA A 和 B（每个任务独立）==========
        # A: Linear(dim, r) → weight: [r, dim]
        # B: Linear(r, dim) → weight: [dim, r]
        # delta_W = B @ M @ A  → [dim, r] @ [r, r] @ [r, dim] = [dim, dim]
        ...
        # ✅ 修正：A_v 应该和 A_k 一样是 Linear(dim, r)
        self.lora_A_v = nn.ModuleList([nn.Linear(dim, r, bias=False) for _ in range(n_tasks)])
        # ✅ 修正：B_v 应该和 B_k 一样是 Linear(r, dim)
        self.lora_B_v = nn.ModuleList([nn.Linear(r, dim, bias=False) for _ in range(n_tasks)])

        # ========== M: rank x rank（每个任务独立）==========
        self.lora_M_k = nn.ModuleList([nn.Linear(r, r, bias=False) for _ in range(n_tasks)])
        self.lora_M_v = nn.ModuleList([nn.Linear(r, r, bias=False) for _ in range(n_tasks)])
    ...
    def init_param(self):
        for t in range(len(self.lora_A_k)):
            ...
            # M: 单位矩阵初始化（使得 B @ I @ A = B @ A）
            nn.init.eye_(self.lora_M_k[t].weight)
            nn.init.eye_(self.lora_M_v[t].weight)
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        ...
        # ========== LoRA with M: delta_W = B @ M @ A ==========
        if task > -0.5:
            weight_k = torch.zeros(C, C, device=x.device)
            weight_v = torch.zeros(C, C, device=x.device)
            
            for t in range(task + 1):
                # A: [r, dim], M: [r, r], B: [dim, r]
                A_k = self.lora_A_k[t].weight
                M_k = self.lora_M_k[t].weight
                B_k = self.lora_B_k[t].weight
                
                A_v = self.lora_A_v[t].weight
                M_v = self.lora_M_v[t].weight
                B_v = self.lora_B_v[t].weight
                
                # delta_W = B @ M @ A
                # B: [dim, r] @ M: [r, r] @ A: [r, dim] = [dim, dim]
                weight_k += B_k @ M_k @ A_k
                weight_v += B_v @ M_v @ A_v
