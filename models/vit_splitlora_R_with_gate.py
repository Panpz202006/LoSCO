...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, 
        ...
        self.Lora_shared_A_k = nn.Linear(dim, r, bias=False)
        self.Lora_shared_B_k = nn.Linear(r, dim, bias=False)
        ...
        # ✅ 新增：共享适配器门控网络（不涉及差分）
        # ============================================================
        # 设计思想：由当前输入特征 x 决定"这个 token 应该贡献多少"
        
        # # 方式1：标量门控（每个 token 一个值）——最轻量
        ...
        # 方式2：Channel-wise 门控（每个维度独立控制）——更细粒度
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_weight=False):
        ...
        # ====== 1. Task-specific LoRA ======
        if task > -0.5:
            weight_k = torch.stack([torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) 
                                   for t in range(task + 1)], dim=0).sum(dim=0)
            weight_v = torch.stack([torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight) 
                                   for t in range(task + 1)], dim=0).sum(dim=0)
        ...
        # gate = self.shared_gate(x_pooled)  # [B, 1]
        # gate = gate.unsqueeze(1)  # [B, 1, 1]
        # # 广播到 [B, 1, 1] × [B, N, C] → 每个 token 共享同一个门控值
        
        # # # 方式B：Channel-wise 门控（更精细，可选）
        ...
        # # gate: [B, 1, 1] → 扩展为 [B, num_heads, N, head_dim]
        # gate_expanded = gate.unsqueeze(2).expand(-1, -1, self.num_heads, -1)  # [B, 1, num_heads, 1]
        # gate_expanded = gate_expanded.permute(0, 2, 1, 3)  # [B, num_heads, 1, 1]
        # # 广播到 [B, num_heads, N, head_dim]
        
        # k = k + gate_expanded * k_shared
        # v = v + gate_expanded * v_shared
        # ====== 方式B：Channel-wise 门控 ======
        gate = self.shared_gate_channel(x)  # [B, N, C]
        ...
        # k = k + gate * k_shared + 1*v_shared # [B, num_heads, N, head_dim]
        # v = v + gate * v_shared + 1*k_shared
        # k = k + gate * k_shared 
        # v = v + gate * v_shared 
        # k = k + gate * k_shared + 1*v_shared # [B, num_heads, N, head_dim]
        # v = v + gate * v_shared + 1*k_shared     
        k = k + gate * v_shared # [B, num_heads, N, head_dim]
        v = v + gate * k_shared     
        # k = k + k_shared 
        # v = v + v_shared 
        # ====== 注意力计算 ======
...
#         shared_weight_k = torch.mm(self.Lora_shared_B_k.weight, self.Lora_shared_A_k.weight)
#         shared_weight_v = torch.mm(self.Lora_shared_B_v.weight, self.Lora_shared_A_v.weight)
#         gate = torch.sigmoid(shared_weight_k)

        
#         k = k + F.linear(x, shared_weight_k).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
#         v = v + F.linear(x, shared_weight_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
