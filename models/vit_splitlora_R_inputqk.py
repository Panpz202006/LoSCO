...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10):
        ...
        self.Lora_shared_A_k = nn.Linear(dim, r, bias=False)  # 权重形状: [r, dim]
        self.Lora_shared_B_k = nn.Linear(r, dim, bias=False)  # 权重形状: [dim, r]
        
        self.Lora_shared_A_v = nn.Linear(dim, r, bias=False)
        self.Lora_shared_B_v = nn.Linear(r, dim, bias=False)
        ...
        self.n_cur_matrix = 0
        self.cur_matrix = torch.zeros(dim ,dim)
        # self.shared_gate_channel = nn.Sequential(
        ...
        self.shared_gate = nn.Sequential(
            # nn.Conv1d(dim, dim, kernel_size=1, padding=0, groups=dim, bias=False),  # 逐通道

            # nn.Conv1d(dim, dim, kernel_size=5, padding=2, groups=dim, bias=False),  # 逐通道
            # nn.Conv1d(dim, dim, kernel_size=3, padding=1, groups=dim, bias=False),  # 逐通道
            nn.Conv1d(dim, dim, kernel_size=3, padding=1, groups=dim, bias=False),  # 逐通道
        ...
        # 参数量: 2 × dim × 3 = 4608（ViT-B/16）
        self.preweight_k = None
        # self.cur_matrix 
        # self.n_cur_matrix 
        # 标量门控：每个 token 一个标量值
        # self.shared_gate = nn.Sequential(
    ...
    def init_param(self):
        for t in range(len(self.lora_A_k)):
            # 这里对 Y=X(W+AB),A和B的初试化方式采用与原始的lora的初始化方式一致，
            # A采用kaiming初始化方式，B采用full zero初始化
            ...
            nn.init.kaiming_uniform_(self.Lora_shared_A_k.weight, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.Lora_shared_A_v.weight, a=math.sqrt(5))

            nn.init.zeros_(self.Lora_shared_B_k.weight)
            nn.init.zeros_(self.Lora_shared_B_v.weight)

            nn.init.zeros_(self.lora_B_k[t].weight)
            nn.init.zeros_(self.lora_B_v[t].weight)
            # nn.init.kaiming_uniform_(self.shared_gate_channel[0].weight, a=math.sqrt(5))
            # nn.init.kaiming_uniform_(self.shared_gate_channel[2].weight, a=math.sqrt(5))
                    # 门控专用初始化
            # nn.init.normal_(self.shared_gate_channel[0].weight, mean=0.0, std=0.01)
            # nn.init.normal_(self.shared_gate_channel[2].weight, mean=0.0, std=0.01)
            # if self.shared_gate_channel[0].bias is not None:
            #     nn.init.zeros_(self.shared_gate_channel[0].bias)
            # if self.shared_gate_channel[2].bias is not None:
            # #     nn.init.zeros_(self.shared_gate_channel[2].bias)
            # nn.init.normal_(self.shared_gate[0].weight, mean=0.0, std=0.01)
            # nn.init.normal_(self.shared_gate[2].weight, mean=0.0, std=0.01)
            nn.init.kaiming_uniform_(self.shared_gate[0].weight, a=math.sqrt(5))
            # nn.init.kaiming_uniform_(self.shared_gate[2].weight, a=math.sqrt(5))
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_weight=False):
        if get_feat:
            self.matrix = (
        ...
        self.preweight_k = self.qkv.weight[out_dim : 2*out_dim, :]
        self.preweight_v = self.qkv.weight[2*out_dim : 3*out_dim, :]

        if get_weight:
            # self.weight_k = k + torch.stack([torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) for t in range(task+1)], dim=0).sum(dim=0)
            # self.weight_v = v + torch.stack([torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight) for t in range(task+1)], dim=0).sum(dim=0)
            ...
            self.weight_k = self.preweight_k + weight_k
            self.weight_v = self.preweight_v + weight_v



        # insert lora
        if task > -0.5:
            ## B 权重形状: [dim, r] @ A 权重形状: [r, dim] #  -> dim,dim
            weight_k = torch.stack([torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) for t in range(task+1)], dim=0).sum(dim=0)
            weight_v = torch.stack([torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight) for t in range(task+1)], dim=0).sum(dim=0)
            # X @ [dim(B),dim(A)]^T = X @ [dim(A),dim(B)]
        ...
        # ✅ 在这里加入shared weights（Lora_shared_v.weight 和 Lora_shared_k.weight）
        # 共享权重：对所有任务通用，用A和B矩阵的乘积

        ## A 权重形状: [r, dim] # B 权重形状: [dim, r] -> dim,dim
        shared_weight_k = torch.mm(self.Lora_shared_B_k.weight, self.Lora_shared_A_k.weight)
        shared_weight_v = torch.mm(self.Lora_shared_B_v.weight, self.Lora_shared_A_v.weight)


        # 逐通道门控
        # gate = self.shared_gate_channel(x)  # [B, N, C]
        # # ====== 将 gate 从 [B, N, C] reshape 到 [B, num_heads, N, head_dim] ======
        # # C = num_heads * head_dim
        # gate = gate.view(B, N, self.num_heads, C // self.num_heads)  # [B, N, num_heads, head_dim]
        # gate = gate.permute(0, 2, 1, 3)  # [B, num_heads, N, head_dim]

        # 标量门控
        # gate = self.shared_gate(x)  # [B, N, 1]
        # # 扩展到 head 维度：[B, N, 1] → [B, num_heads, N, 1]
        # gate = gate.unsqueeze(1)  # [B, 1, N, 1]
        # gate = gate.expand(B, self.num_heads, N, C // self.num_heads)  # [B, num_heads, N, head_dim

       # ========== 🆕 1D 卷积门控 ==========
        # x: [B, N, C] → permute → [B, C, N] → Conv1d → [B, C, N] → permute → [B, N, C]
        # gate = self.shared_gate(x.permute(0, 2, 1)).permute(0, 2, 1)  # [B, N, C]

        # # 将 gate reshape 到多头格式: [B, N, C] → [B, N, num_heads, head_dim] → [B, num_heads, N, head_dim]
        # gate = gate.view(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # [B, num_heads, N, head_dim]
        

                # ====== 应用门控到共享权重 ======
        k_shared = F.linear(x, shared_weight_k).reshape(
            B, N, self.num_heads, C // self.num_heads
        ).permute(0, 2, 1, 3)  # [B, num_heads, N, head_dim]

        v_shared = F.linear(x, shared_weight_v).reshape(
            B, N, self.num_heads, C // self.num_heads
        ).permute(0, 2, 1, 3)  # [B, num_heads, N, head_dim]


        # k = k + gate *k_shared + k_shared
        # v = v + gate *v_shared + v_shared

        # k = k + gate *k_shared + v_shared
        # v = v + gate *v_shared + k_shared
        ...
        # k = (1-gate)*k + gate *k_shared 
        # v = (1-gate)*v + gate *v_shared 
        gate_input = k_shared.permute(0, 2, 1, 3).reshape(B, N, C)
        gate = self.shared_gate(gate_input.permute(0, 2, 1)).permute(0, 2, 1)
        gate = gate.view(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = k + gate * k_shared
        v = v + gate * v_shared
...
class Block(nn.Module):
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_weight=False):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), task, register_hook=register_hook, get_feat=get_feat, get_cur_feat=get_cur_feat, get_weight=get_weight)))
