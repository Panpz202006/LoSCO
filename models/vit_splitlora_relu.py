...
class Attention_LoRA(nn.Module):
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        ...
        # if task > -0.5:
        #     # y = ReLU(xA) B
        #     k = k + self.lora_B_k[task](torch.sigmoid(self.lora_A_k[task](x))).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        #     v = v + self.lora_B_v[task](torch.sigmoid(self.lora_A_v[task](x))).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # if task > -0.5:
        #     # 累加所有权重矩阵
        #     weight_k = torch.zeros(self.lora_B_k[0].weight.shape[1], self.lora_A_k[0].weight.shape[0])
        #     weight_v = torch.zeros_like(weight_k)
        #     for t in range(task + 1):
        #         weight_k = weight_k + self.lora_B_k[t].weight @ self.lora_A_k[t].weight
        #         weight_v = weight_v + self.lora_B_v[t].weight @ self.lora_A_v[t].weight
        #     k = k + F.linear(x, weight_k).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        #     v = v + F.linear(x, weight_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        if task > -0.5:
            # 累加所有任务 0 到 task 的 LoRA 贡献
            k_contrib = torch.zeros_like(x)
            v_contrib = torch.zeros_like(x)
            for t in range(task + 1):
                # k_contrib = k_contrib + self.lora_B_k[t](F.relu(self.lora_A_k[t](x)))
                # v_contrib = v_contrib + self.lora_B_v[t](F.relu(self.lora_A_v[t](x)))
                k_contrib = k_contrib + self.lora_B_k[t](torch.sigmoid(self.lora_A_k[t](x)))
                v_contrib = v_contrib + self.lora_B_v[t](torch.sigmoid(self.lora_A_v[t](x)))                
            k = k + k_contrib.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            v = v + v_contrib.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
