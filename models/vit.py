...
class Attention_LoRA(nn.Module):
    ...
    def forward(self, x, use_new=True):
        ...
        if use_new:
            new_k = self.lora_new_B_k(self.lora_new_A_k(x))
            new_v = self.lora_new_B_v(self.lora_new_A_v(x))
            qkv[:, :, self.dim : 2*self.dim] += new_k
            qkv[:, :, 2*self.dim :] += new_v
...
class Block(nn.Module):
    ...
    def forward(self, x, use_new=True):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), use_new=use_new)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            block_fn: (nn.Module): transformer block function
            n_tasks: (int): number of continual learning tasks
            rank: (int): rank of LoRA modules
        """
        super().__init__()
        ...
        # class token + positional embedding + dropout layer
