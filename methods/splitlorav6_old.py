...
class SplitLoRAV6(BaseLearner):
    ...
    def _train(self, train_loader):
        ...
        if self.cur_task==0:
            ...
            # self._train_function_from_one(train_loader, optimizer, scheduler)
            self._train_function(train_loader, optimizer, scheduler)
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                for p in trainable_params:
                    ...
                    p.data.add_(mu * z)
                # 4. 正扰动前向传播
                ret_plus = self.network(inputs)
                loss_plus = F.cross_entropy(ret_plus['logits'], targets)
                ...
                # 7. 🆕 ZO梯度估计：ĝ = (L⁺ - L⁻) / (2μ) * z
                #    直接用估计的梯度更新，不需要 loss.backward()！
                grad_scale = (loss_plus - loss_minus) / (2 * mu)                
                with torch.no_grad():
                    for p, z in zip(trainable_params, perturbations):
                        # θ ← θ - η * ĝ = θ - η * (L⁺ - L⁻) / (2μ) * z
                        p.data.sub_(self.args["lr"] * grad_scale * z)
    ...
    def _train_function_from_one(self, train_loader, optimizer, scheduler):
        if self.cur_task >= 1:
            ema_keys = ["Lora_shared_B_k", "Lora_shared_B_v", "Lora_shared_A_k", "Lora_shared_A_v"]
            ema_decay = 0.9999
            ema_params = {}
        ...
        for name, param in self.network.named_parameters():
            if param.requires_grad:
                if 'classifier_pool' in name:
                    param_lr_pairs.append((param, self.lrate_head))
                else:
                    param_lr_pairs.append((param, self.lrate))
        
        # 分离出参数列表和对应的学习率（供扰动使用）
        trainable_params = [pair[0] for pair in param_lr_pairs]
        param_lrs = [pair[1] for pair in param_lr_pairs]
        # =========================================
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # ✅ 使用预先构建的 param_lr_pairs 更新
                with torch.no_grad():
                    for (p, lr), z in zip(param_lr_pairs, perturbations):
                        p.data.sub_(lr * grad_scale * z)
                # =========================================
