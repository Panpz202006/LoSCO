...
class SplitLoRAV6(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.alpha_smooth = {}           # 存储每个参数的平滑 alpha
        self.alpha_smooth_decay = 0.9    # alpha 平滑系数（越大越平滑）        
                ...
                if self.cur_task >= 1:
                    ...
                    # elif self.ema_type == "grad_direction":  # <--- 新增：梯度方向一致性EMA
                        ...
                        for name, param in self.network.named_parameters():
                            ...
                            # ---------- 稳定映射：cos_sim ∈ [-1,1] → alpha ∈ [0.9, 0.99999] ----------
                            # 1. 归一化到 [0, 1]
                            normalized = (cos_sim + 1) * 0.5
                            
                            # 2. 用 sigmoid 映射到 [0.9, 0.99999]，敏感度系数 3.0
                            ...
                            print(f"cur_task: {self.cur_task}, alpha_star: {self.alpha_smooth[name]}")
                            alpha_star = torch.clamp(self.alpha_smooth[name], 0.9, 0.99999)
                            print(f"cur_task: {self.cur_task}, revised alpha_star: {self.alpha_smooth[name]}")

                            # ---------- 参数EMA更新 ----------
                            ema_params[name] = alpha_star * ema_params[name] + (1 - alpha_star) * theta_t
                            prev_ema_params[name] = ema_params[name].clone().detach()

                    elif self.ema_type == "curvature_adaptive":
                        for name, param in self.network.named_parameters():
                            if name not in ema_params:
                                continue
                            if param.grad is None:
                            ...
                            # 曲率近似：梯度变化 / 参数变化
                            grad_cur = param.grad
                            if name in prev_grad_params:
                                grad_prev = prev_grad_params[name]
                                grad_diff = torch.norm(grad_cur - grad_prev)
                                param_diff = torch.norm(theta_t - bar_theta_prev)
                                
                                if param_diff > 1e-8:
                                    curvature = grad_diff / (param_diff + 1e-8)
                                else:
                                    curvature = torch.tensor(0.0, device=param.device)
                            else:
                                curvature = torch.tensor(0.0, device=param.device)
                                prev_grad_params[name] = grad_cur.clone().detach()
                                continue
                            
                            # 曲率映射到 alpha
                            # 曲率大 → alpha大（稳定），曲率小 → alpha小（可塑）
                            # 用 sigmoid 把曲率映射到 [0.99, 0.99999]
                            # 需要两个超参数：scale（缩放）和 shift（偏移）
                            scale = 1.0
                            shift = 0.5
                            alpha_star = torch.sigmoid(scale * (curvature - shift))
                            alpha_star = 0.99 + (0.99999 - 0.99) * alpha_star
