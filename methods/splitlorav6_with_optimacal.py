    ...
    def _train_function(self, train_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # 作用：强制共享适配器与任务特定适配器的列空间正交。
                # 约束对象是 B 的列空间：‖B_sharedᵀ·B_task‖_F²（[r,r] 重叠矩阵）。
                # 之前两版约束 ΔW=B@A，被冻结的小 A 压制到 ~1e-4 量级（no-op）；
                # B 直接可训练，量级真实、梯度直达。K/V 都约束、从 task 1 开始。
                ...
                #                 B_shared = module.Lora_shared_B_v # 每个任务单独约束，再按任务数平均，避免 loss 随任务数增长 
                #             proj_loss = 0.0 
                ...
                #             layer_count += 1 # 按层平均，避免损失随层数/投影数线性增长
                ...
                # Goal: task-specific adapter and shared adapter are orthogonal
                # ============================================================
                ...
                # Goal: task-specific adapter and shared adapter
                #       have similar parameter updates
                # ============================================================
