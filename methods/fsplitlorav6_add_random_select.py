        ...
        if self.cur_task == 0:
            ...
            for module in self.network.modules():
                if isinstance(module, Attention_LoRA):
                    ...
                    # 生成索引并直接放到 GPU
                    if feature_dim >= rank:
                        indices = torch.randperm(feature_dim, device=self.device)[:rank]
                    else:
                        indices = torch.randint(0, feature_dim, (rank,), device=self.device)

                    # 现在所有操作都在 GPU 上
                    selected_features = self.feature_list[i][:, indices]  # [n_samples, 10]

                    # 生成随机矩阵 [10, 10] 并应用到 selected_features
                    random_matrix_K = torch.rand(rank, rank, device=self.device)  # 均匀分布 [0,1)
                    # 或者用高斯分布
                    random_matrix_V = torch.randn(rank, rank, device=self.device)

                    # 线性变换 [n_samples, 10] @ [10, 10] = [n_samples, 10]
                    selected_features_K = selected_features @ random_matrix_K
                    selected_features_V = selected_features @ random_matrix_V

                    initialized_matrix_V = selected_features_V.clone()
                    initialized_matrix_K = selected_features_K.clone()
