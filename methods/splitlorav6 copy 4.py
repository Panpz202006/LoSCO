    ...
    def _train_function(self, train_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # # 1
                # if self.cur_task >= 1:
                #     # 计算当前任务更新与历史权重的余弦相似度损失
                #     cosine_loss = 0.0
                #     total_pairs = 0
                    
                #     # 正确遍历：通过 network 的 modules() 找到所有 Attention_LoRA
                #     for module in self.network.modules():
                #         if isinstance(module, Attention_LoRA):
                #             attn = module
                            
                #             # 获取当前任务的 LoRA 更新 (AtBt)
                #             delta_k = attn.lora_B_k[self.cur_task].weight @ attn.lora_A_k[self.cur_task].weight
                #             delta_v = attn.lora_B_v[self.cur_task].weight @ attn.lora_A_v[self.cur_task].weight
                            
                #             # 获取历史累积权重 W_{t-1} = W0 + sum_{i=1}^{t-1} A_i B_i
                #             if self.cur_task == 0:
                #                 # 第一个任务，只有原始权重
                #                 W_prev_k = attn.preweight_k
                #                 W_prev_v = attn.preweight_v
                #             else:
                #                 # 计算历史累积的 LoRA 更新
                #                 hist_lora_k = sum(attn.lora_B_k[i].weight @ attn.lora_A_k[i].weight 
                #                                 for i in range(self.cur_task))
                #                 hist_lora_v = sum(attn.lora_B_v[i].weight @ attn.lora_A_v[i].weight 
                #                                 for i in range(self.cur_task))
                                
                #                 W_prev_k = attn.preweight_k + hist_lora_k
                #                 W_prev_v = attn.preweight_v + hist_lora_v
                            
                #             # 计算余弦相似度损失
                #             cosine_loss += self.cosine_similarity_loss(W_prev_k, delta_k)
                #             cosine_loss += self.cosine_similarity_loss(W_prev_v, delta_v)
                #             total_pairs += 2
                    
                #     # 平均损失
                #     if total_pairs > 0:
                #         cosine_loss = cosine_loss / total_pairs
                #         loss += 0.001 * cosine_loss  # 调整系数
                #  2.
                # if self.cur_task >= 1:
                #     # 鼓励当前任务更新与历史权重相似
                #     similarity_loss = 0.0
                ...
                #             # 鼓励相似：1 - cos_sim，相似度越高损失越小
                #             similarity_loss += (1 - self.cosine_similarity(W_prev_k, delta_k))
                #             similarity_loss += (1 - self.cosine_similarity(W_prev_v, delta_v))
                #             total_pairs += 2
                    
                #     if total_pairs > 0:
                #         similarity_loss = similarity_loss / total_pairs
                #         loss += 0.0001 * similarity_loss
                ...
                #             ortho_loss += torch.norm(delta_W , p='fro') ** 2

                #     #############################################################################
                #     # loss += 0.001 * ortho_loss * 0.5
                ...
                if self.cur_task>=1 and epoch < 7:
                    # ✅ DEMA更新（改动最小，只改了这里）
                    for name, param in self.network.named_parameters():
