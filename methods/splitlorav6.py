...
class SplitLoRAV6(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.ortho_loss_weight = args.get("ortho_loss_weight", 0.01)
        self.cos_loss_weight = args.get("cos_loss_weight", 0.01)
        self.same_cos_loss_weight = args.get("same_cos_loss_weight", 0.01)
        self.same_loss_weight = args.get("same_loss_weight", 0.01)

        self.cos_loss_type = args.get('cos_loss_type', False)
        self.same_loss_type = args.get('same_loss_type', False)


        self.ortho_loss_sum = 0.0
        self.ortho_batches = 0

        self.cos_loss_sum = 0.0
        self.cos_batches = 0

        self.same_cos_loss_sum = 0.0
        self.same_cos_batches = 0

        self.same_loss_sum = 0.0
        self.same_batches = 0

        self.same_cos_loss_type = args.get('same_cos_loss_type', False)
        # 当前训练步数（用于cosine递增）
        ...
        self.flat_cosine_anneal = args.get('flat_cosine_anneal', False)
        ...
        self.ortho_loss_typev2 = args.get('ortho_loss_typev2', False)
            ...
            # ========== Focal Loss 相关配置 ==========
        self.use_focal_loss = args.get('use_focal_loss', False)  # 是否使用Focal Loss
        self.focal_alpha = args.get('focal_alpha', 0.25)  # 平衡因子，默认0.25
        self.focal_gamma = args.get('focal_gamma', 2.0)   # 聚焦参数，默认2.0
        self.focal_alpha_schedule = args.get('focal_alpha_schedule', False)  # 是否动态调整alpha
    ...
    # def _compute_focal_loss(self, logits, targets, alpha=None, gamma=None):
    #     """
    #     计算Focal Loss
    ...
    #         alpha: 平衡因子，默认使用 self.focal_alpha
    #         gamma: 聚焦参数，默认使用 self.focal_gamma
    ...
    #         # 获取每个样本的alpha权重
    #         alpha_t = torch.ones_like(targets, dtype=torch.float) * alpha
    #         # 对于真实类别，使用alpha；其他类别使用1-alpha
    #         # 这里简化处理：直接使用alpha_t * (1 - pt)^gamma * ce_loss
    ...
    def _compute_focal_loss(self, logits, targets, alpha=None, gamma=None):
        ...
        focal_loss = (1 - pt).pow(gamma) * ce_loss
    ...
    def _train_function(self, train_loader, optimizer, scheduler):
        # ========== EMA初始化（原有逻辑，保持不变） ==========
        if self.cur_task >= 1:
            # ema_keys = ["Lora_shared_B_k", "Lora_shared_B_v", "Lora_shared_A_k", "Lora_shared_A_v", "shared_gate_channel"]
            ema_keys = ["Lora_shared_B_k", "Lora_shared_B_v", "Lora_shared_A_k", "Lora_shared_A_v"]
            ema_gate_keys = ["shared_gate_channel"]
            
            # ema_gate_keys = ["shared_gate"]
            ...
            if self.ema_type == "adaptEMA":
                ...
                if self.flag ==1:
                    ...
                    print(f"curent task {self.cur_task}, ema decay {ema_decay}")
                    self.ema_params = {}
                    self.ema_keys_params={}
                    for name, param in self.network.named_parameters():
                        if any(key in name for key in ema_keys):
                            self.ema_params[name] = param.data.clone().detach()
                            print(f"EMA tracking: {name}")
                        if any(key in name for key in ema_gate_keys):
                            self.ema_keys_params[name] = param.data.clone().detach()
                            print(f"EMA tracking: {name}")
                    # self.flag = 1
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # loss = loss_minus
                loss_minus = self._compute_focal_loss(logits, targets, alpha=self.focal_alpha, gamma=self.focal_gamma)
                loss = loss_minus
                ...
                #                 # 计算余弦相似度而不是F范数重叠
                #                 B_shared_flat = B_shared.weight.view(-1)
                #                 B_task_flat = B_tasks[t].weight.view(-1)
                                
                #                 cos_sim = torch.dot(B_shared_flat, B_task_flat) / (
                #                     torch.norm(B_shared_flat) * torch.norm(B_task_flat) + 1e-8
                #                 )
                                
                #                 # 使用 (1 + cos_sim) 作为重叠度，范围 [0, 2]
                #                 # 或者直接用 |cos_sim| 作为重叠度
                #                 overlap = torch.abs(cos_sim)  # 范围 [0, 1]
                                
                #                 # 正交损失：鼓励cos_sim接近0
                #                 proj_loss += cos_sim ** 2  # 或者 (1 - cos_sim**2)
                #                 proj_overlap += overlap.detach()

                #             ortho_loss += proj_loss / (self.cur_task + 1)
                #             overlap_sum += proj_overlap / (self.cur_task + 1)
                #             layer_count += 1

                #     ortho_loss /= layer_count
                #     overlap_avg = overlap_sum / layer_count
                    
                #     # EMA平滑
                #     m = 0.5
                #     self.overlap_ema = m * self.overlap_ema + (1-m) * overlap_avg.item()
                    
                #     # 动态权重：overlap越大，惩罚越强
                #     # lambda_min = 0.01
                #     # lambda_max = self.ortho_loss_weight
                #     # dynamic_weight = lambda_min + (lambda_max - lambda_min) * self.overlap_ema
                #     dynamic_weight = self.overlap_ema

                #     print(f"self.curtask:{self.cur_task}, self.epoch:{epoch},overlap_avg:{overlap_avg} ,dynamic_weight:{dynamic_weight}")

                #     loss += dynamic_weight * ortho_loss
                # 2.
                # if self.ortho_loss_type and self.cur_task >= 1:
                #     ortho_loss = 0.
                #     overlap_sum = 0.
                ...
                #                 cross = B_shared.weight.T @ B_tasks[t].weight

                #                 cross_norm = torch.norm(cross, p='fro') ** 2

                #                 Bs_norm = torch.norm(B_shared.weight, p='fro') ** 2
                #                 Bt_norm = torch.norm(B_tasks[t].weight, p='fro') ** 2

                #                 overlap = cross_norm / (Bs_norm * Bt_norm + 1e-8)

                #                 proj_loss += cross_norm
                #                 proj_overlap += overlap.detach()
                ...
                #     # ---------- adaptive lambda ----------
                #     lambda_min = 0.01
                #     lambda_max = self.ortho_loss_weight   # 例如0.12

                #     dynamic_weight = self.overlap_ema
                #     print(f"self.curtask:{self.cur_task}, self.epoch:{epoch},overlap_avg:{overlap_avg} ,dynamic_weight:{dynamic_weight}")
                #     loss += self.overlap_ema * ortho_loss
                # 1
                # if self.ortho_loss_type and self.cur_task >= 1:
                #     ortho_loss = 0.0
                #     layer_count = 0
                #     overlap_avg = 0.0  # 记录平均重叠度
                ...
                #             for t in range(self.cur_task + 1):
                #                 cross = B_shared.weight.T @ B_tasks[t].weight
                #                 overlap = torch.norm(cross, p='fro') ** 2
                #                 proj_loss += overlap
                #                 proj_overlap += overlap.item()
                            
                #             ortho_loss += proj_loss / (self.cur_task + 1)
                #             overlap_avg += proj_overlap / (self.cur_task + 1)
                #             layer_count += 1

                #     if layer_count > 0:
                #         ortho_loss = ortho_loss / layer_count
                #         overlap_avg = overlap_avg / layer_count

                #     # ===== 核心改动：根据重叠度动态调整权重 =====
                #     # overlap_avg 越大，说明共享和任务参数重叠越多，需要加强正交约束
                #     dynamic_weight = self.ortho_loss_weight * (1.0 + overlap_avg)
                #     # 限制范围避免爆炸
                #     dynamic_weight = min(max(dynamic_weight, 0.01), 1.0)
                #     print(f"self.curtask:{self.cur_task}, self.epoch:{epoch},overlap_avg:{overlap_avg} ,dynamic_weight:{dynamic_weight}")
                #     loss += dynamic_weight * ortho_loss
                    #############################################################################        
                # 原始的方案
                if self.ortho_loss_type and self.cur_task >= 1:
                    ...
                    for module in self.network.modules():
                        ...
                        for proj in ('k', 'v'):
                            ...
                            for t in range(self.cur_task + 1):
                                ...
                                # # identity = torch.eye(cross.size(0), device=cross.device)
                                # # 添加范数约束，防止坍缩到零
                                # norm_shared = torch.norm(B_shared.weight, p='fro') ** 2
                                # norm_task = torch.norm(B_tasks[t].weight, p='fro') ** 2
                                # proj_loss += 0.001 * (norm_shared - self.rank) ** 2 + 0.001 * (norm_task - self.rank) ** 2
                                # # proj_loss += 1 * (norm_shared - self.rank) ** 2 + 1 * (norm_task - self.rank) ** 2

                                # # proj_loss = torch.norm(G - identity, p='fro') ** 2
                                # # proj_loss += torch.norm(cross - identity, p='fro') ** 2
                                proj_loss += torch.norm(cross, p='fro') ** 2
                                # B_shared_norm = F.normalize(B_shared.weight, dim=0)  # 列归一化
                                # B_t_norm = F.normalize(B_tasks[t].weight, dim=0)     # 列归一化

                                # # 约束归一化后的列正交
                                # cross = B_shared_norm.T @ B_t_norm
                                # proj_loss = torch.norm(cross, p='fro') ** 2
                            ortho_loss += proj_loss / (self.cur_task + 1)
                            layer_count += 1
                    ...
                    ortho_loss_sum += self.ortho_loss_weight * ortho_loss.item()
                    ortho_loss_sum += ortho_loss.item()

                    ortho_batches += 1

                if self.cos_loss_type and self.cur_task >= 1:
                    #############################################################################
                    cos_loss = 0.0
                    layer_count = 0
                    ...
                    for module in self.network.modules():
                        ...
                        for proj in ('k', 'v'):
                            ...
                            if proj == 'k':
                                B_tasks = module.lora_B_k
                                A_tasks = module.lora_A_k
                                B_shared = module.Lora_shared_B_k
                                A_shared = module.Lora_shared_A_k
                            else:
                                B_tasks = module.lora_B_v
                                A_tasks = module.lora_A_v
                                B_shared = module.Lora_shared_B_v
                                A_shared = module.Lora_shared_A_v

                            # Shared adapter
                            delta_W_shared = (
                                B_shared.weight @ A_shared.weight
                            )

                            norm_shared = torch.norm(
                                delta_W_shared,
                                p='fro'
                            )
                            ...
                            for t in range(self.cur_task + 1):
                                ...
                                delta_W_task = (
                                    B_tasks[t].weight @ A_tasks[t].weight
                                )

                                norm_task = torch.norm(
                                    delta_W_task,
                                    p='fro'
                                )

                                if norm_task > 1e-8 and norm_shared > 1e-8:

                                    # Frobenius inner product
                                    frobenius_inner = torch.sum(
                                        delta_W_task * delta_W_shared
                                    )

                                    cos_sim = (
                                        frobenius_inner
                                        / (norm_task * norm_shared + 1e-8)
                                    )

                                    # Encourage cosine similarity -> 0
                                    proj_loss += cos_sim ** 2

                            # 按 task 数量归一化
                            proj_loss = proj_loss / (self.cur_task + 1)

                            # K/V projection 作为一个 layer/projection 单元
                            cos_loss += proj_loss
                            layer_count += 1

                    # 按 Attention_LoRA 的 K/V projection 数量归一化
                    if layer_count > 0:
                        cos_loss = cos_loss / layer_count

                    #############################################################################
                    loss += self.cos_loss_weight * cos_loss
                ...
                # Goal: task-specific adapter and shared adapter
                #       have similar directions
                # ============================================================
                if self.same_cos_loss_type and self.cur_task >= 1:
                    #############################################################################
                    same_cos_loss = 0.0
                    layer_count = 0
                    ...
                    for module in self.network.modules():
                        ...
                        for proj in ('k', 'v'):
                            ...
                            same_cos_loss += proj_loss
                            layer_count += 1

                    # K/V + layer normalization
                    if layer_count > 0:
                        same_cos_loss = same_cos_loss / layer_count

                    #############################################################################
                    loss += self.same_cos_loss_weight * same_cos_loss

                    same_cos_loss_sum += same_cos_loss.item()
                    same_cos_batches += 1




                if self.same_loss_type and self.cur_task >= 1:
                    #############################################################################
                    same_loss = 0.0
                    layer_count = 0
                    ...
                    # K/V + layer normalization
                    if layer_count > 0:
                        same_loss = same_loss / layer_count

                    #############################################################################
                    loss += self.same_loss_weight * same_loss

                    same_loss_sum += same_loss.item()
                    same_batches += 1
                ...
                if self.cur_task >= 1:
                    if self.ema_type == "EMA2":
                        ...
                        for name, param in self.network.named_parameters():
                            ...
                            if name in self.ema_keys_params:
                                ema_gate_decay = 0.5
                                self.ema_keys_params[name] = (ema_gate_decay * self.ema_keys_params[name] + (1 - ema_gate_decay) * param.data)
                ...
                with torch.no_grad():
                    for name, param in self.network.named_parameters():
                        if name in self.ema_params:
                            param.data.copy_(self.ema_params[name])
                        if name in self.ema_keys_params:
                            param.data.copy_(self.ema_keys_params[name])
