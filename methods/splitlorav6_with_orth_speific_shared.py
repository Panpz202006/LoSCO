    ...
    def _train_function(self, train_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                # 作用：强制任务特定适配器总和与共享适配器矩阵乘积为零。
                if self.ortho_loss_type: #所有任务都计算
                    #############################################################################
                    ortho_loss = 0.0
                    ...
                    for module in self.network.modules():
                        if isinstance(module, Attention_LoRA):
                            ...
                            for task_id in range(self.cur_task + 1):
                                delta_W_task_sum += module.lora_B_k[task_id].weight @ module.lora_A_k[task_id].weight
                            
                            # 共享适配器
                            delta_W_shared = module.Lora_shared_B_k.weight @ module.Lora_shared_A_k.weight
                            
                            ortho_loss += torch.norm(delta_W_task_sum @ delta_W_shared.T, p='fro') ** 2
                    
                    #############################################################################
                    loss += 0.001 * ortho_loss

                # 作用：强制任务特定适配器总和与共享适配器的方向尽量不同（余弦相似度趋近于0）。
                if self.cos_loss_type: #使用余弦相似度损失
                    #############################################################################
                    cos_loss = 0.0
                    layer_count = 0
                    ...
                    for module in self.network.modules():
                        if isinstance(module, Attention_LoRA):
                            ...
                            # 计算Frobenius内积（矩阵版本的余弦相似度）
                            frobenius_inner = torch.sum(delta_W_task_sum * delta_W_shared)
                            norm_task = torch.norm(delta_W_task_sum, p='fro')
                            norm_shared = torch.norm(delta_W_shared, p='fro')
                            
                            if norm_task > 1e-8 and norm_shared > 1e-8:
                                cos_sim = frobenius_inner / (norm_task * norm_shared)
                                cos_loss += cos_sim ** 2
                                layer_count += 1
