...
class SplitLoRAV6(BaseLearner):
    ...
    def linear_decay(self, start, end, t, T):
        """线性衰减"""
        return start + (end - start) * (t / T)

    def cosine_decay(self, start, end, t, T):
        """余弦衰减"""
        cos_val = (1 - math.cos(math.pi * t / T)) / 2
        return start + (end - start) * cos_val
    ...
    def _train_function(self, train_loader, optimizer, scheduler):
        ...
        init_ortho_loss_weight = self.ortho_loss_weight 
        global_step = 0
        total_steps = self.epochs * len(train_loader)
        # init_ortho_loss_weight 随着epoch 或者iteration 进行衰减
        for _, epoch in enumerate(prog_bar):
            ...
            ortho_weight_current = self.cosine_decay(0.1, 0.0001, epoch, 10)
            for i, (_, inputs, targets) in enumerate(train_loader):
                inputs, targets = inputs.to(self.device), targets.to(self.device)
                ...
                # import math
                # cos_anneal = (1 + math.cos(math.pi * progress)) / 2
                # ortho_weight_current = init_ortho_loss_weight * cos_anneal 
                # min_ortho_weight = 0.05  # 最低保持
                # ortho_weight_current = max(ortho_weight_current, min_ortho_weight)
                ...
                if self.ortho_loss_type and self.cur_task >= 1:
                    ...
                    #############################################################################
                    loss += ortho_weight_current * ortho_loss
