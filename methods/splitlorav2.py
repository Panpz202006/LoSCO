...
from models.net_splitlora_qkv import Net
from models.vit_splitlora_qkv import VisionTransformer, PatchEmbed, Block, Attention_LoRA
from methods.base import BaseLearner
from utils.toolkit import tensor2numpy
from utils.toolkit import print_trainable_params, check_params_consistency


class SplitLoRAV2(BaseLearner):
        ...
        if self.cur_task == 0:
            ...
            for module in self.network.modules():
                if isinstance(module, Attention_LoRA):
                    # print(f"*************{i}**********")
                    random_coeffs = torch.randn(self.feature_list[i].shape[1],self.rank).to(module.lora_A_q[self.cur_task].weight.device)
                    # 第2行：用特征矩阵乘以随机系数
                    ...
                    # add Q
                    initialized_matrix_Q = initialized_matrix / initialized_matrix.norm() * module.lora_A_q[self.cur_task].weight.data.norm() * (1/(self.cur_task+1))
                    ...
                    # add Q
                    module.lora_A_q[self.cur_task].weight.data.copy_(initialized_matrix_Q.T)
                    i += 1
