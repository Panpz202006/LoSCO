        ...
        if self.cur_task == 0:
            ...
            for module in self.network.modules():
                if isinstance(module, Attention_LoRA):
                    ...
                    random_coeffs_K = nn.Parameter(torch.randn(self.feature_list[i].shape[1],self.rank).to(module.lora_A_k[self.cur_task].weight.device))
                    random_coeffs_V = nn.Parameter(torch.randn(self.feature_list[i].shape[1],self.rank).to(module.lora_A_k[self.cur_task].weight.device))
