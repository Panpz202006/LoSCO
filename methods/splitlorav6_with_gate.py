            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self.cur_task >= 1:
                    if self.ema_type == "EMA2":
                        ...
                        for name, param in self.network.named_parameters():
                            # ema_params = ["Lora_shared_A_k", "Lora_shared_B_k", "Lora_shared_B_v", "Lora_shared_A_v"]
                            ...
                            if name in self.ema_params:
                                ...
                                # diff = self.ema_params[name] - param.data
                                # self.ema_params[name] = self.ema_params[name] + (1 - ema_decay) * (torch.sigmoid(diff) + (diff) )
