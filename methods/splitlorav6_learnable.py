...
class SplitLoRAV6(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.rank = args['rank']
        # self.atl = agrs['atl']
        self._prepare_random_coeffs()
    def _prepare_random_coeffs(self):
        """为每个 Attention_LoRA 模块准备 random_coeffs 的 ParameterDict"""
        for module in self.network.modules():
            if isinstance(module, Attention_LoRA):
                # 为每个模块创建 ParameterDict
                if not hasattr(module, 'random_coeffs_K'):
                    module.random_coeffs_K = nn.ParameterDict()
                    module.random_coeffs_V = nn.ParameterDict()
        ...
        if self.cur_task == 0:
            ...
            for module in self.network.modules():
                if isinstance(module, Attention_LoRA):
                    ...
                    # random_coeffs_V = 
                    feature_dim = self.feature_list[i].shape[1]
                    
                    # 为当前模块的当前任务创建可学习参数
                    task_key = str(self.cur_task)
                    
                    # 如果当前任务的参数还没创建，则创建
                    # if task_key not in module.random_coeffs_K:
                        # # 使用 torch.ones 或 torch.randn 初始化
                    module.random_coeffs_K[task_key] = nn.Parameter(
                        torch.ones(feature_dim, self.rank).to(self.device)
                    )
                    module.random_coeffs_V[task_key] = nn.Parameter(
                        torch.ones(feature_dim, self.rank).to(self.device)
                    )
                    # module.random_coeffs_K[task_key] = nn.Parameter(
                    #     torch.empty(feature_dim, self.rank, device=self.device)
                    # )
                    # module.random_coeffs_V[task_key] = nn.Parameter(
                    #     torch.empty(feature_dim, self.rank, device=self.device)
                    # )
                    # from torch.nn import init
                    # # Kaiming 均匀初始化（默认 a=0, mode='fan_in', nonlinearity='leaky_relu'）
                    # init.kaiming_uniform_(module.random_coeffs_K[task_key], a=0, mode='fan_in', nonlinearity='leaky_relu')
                    # init.kaiming_uniform_(module.random_coeffs_V[task_key], a=0, mode='fan_in', nonlinearity='leaky_relu')
                    
                    # 获取参数
                    random_coeffs_K = module.random_coeffs_K[task_key]
                    random_coeffs_V = module.random_coeffs_V[task_key]                    
                    # random_coeffs_K = nn.Parameter(torch.ones(self.feature_list[i].shape[1],self.rank).to(module.lora_A_k[self.cur_task].weight.device))
                    # random_coeffs_V = nn.Parameter(torch.ones(self.feature_list[i].shape[1],self.rank).to(module.lora_A_k[self.cur_task].weight.device))
    ...
    def freeze_network(self):
        ...
        unfrozen_keys = [
            ...
            f"scaling_factor",
        f"random_coeffs_K{target_suffix}",  # 只解冻当前任务的
        f"random_coeffs_V{target_suffix}",
